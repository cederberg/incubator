...
def test_atx_levels_1_to_6():
    ...
    items = parse(text)
    assert [it.signature for it in items] == [f"{'#' * i} H{i}" for i in range(1, 7)]
...
def test_atx_fixture():
    ...
    assert any("Title" in s for s in sigs)
    assert any("Section One" in s for s in sigs)
    assert any("Subsection A" in s for s in sigs)
...
def test_mixed_fixture():
    ...
    items = parse(text)
    sigs = [it.signature for it in items]
    assert any("ATX Title" in s for s in sigs)
    assert any("Section Alpha" in s for s in sigs)
    assert any("ATX Sub" in s for s in sigs)
    assert any("Beta Section" in s for s in sigs)
