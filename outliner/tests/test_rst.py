...
from outliner.parsers.rst import parse as _parse, detect as detect_rst
...
def test_detect_rst_extension_overrides_content():
    # .rst extension takes priority; content need not have RST-specific markers
    assert guess_syntax("README.rst") == "rst"
    assert guess_syntax("README.rest") == "rst"


# ---------------------------------------------------------------------------
# Overline + underline headings
# ---------------------------------------------------------------------------

def test_overline_underline_no_spurious_preamble():
    # Sphinx-style overline+underline: the overline line must not become a
    # phantom preamble item; only the heading itself should appear.
    text = "=========\nChapter 1\n=========\n\nBody text.\n"
    items = parse(text)
    assert len(items) == 1
    assert items[0].signature == "Chapter 1"


def test_overline_underline_mixed_with_plain_underline():
    # Overline title followed by a normal underline section.
    text = "=========\nChapter 1\n=========\n\nSection\n-------\n\ntext\n"
    items = parse(text)
    sigs = [it.signature for it in items]
...
# ---------------------------------------------------------------------------
# min_len: underline must cover stripped title width
# ---------------------------------------------------------------------------

def test_underline_matches_stripped_title_length():
    # Title with leading whitespace: underline just long enough for the text
    # (not counting the leading space) must still be accepted.
    text = " Title\n======\n\nbody\n"
    items = parse(text)
    assert len(items) == 1
    assert items[0].signature == "Title"
