...
scripts (`./mvnw`, `./gradlew`) over direct tool invocation. Use the toolchain's
clean command, or fall back to `rm -rf <build output dirs>`.
...
- Use per-filetype sections to limit rule scope (e.g. `## Go Code`,
  `## Shell Scripts`)
