...
Report the first occurrence of each violation. If problems recur, add after the
list: `Recurring: #{{N}}, #{{M}} — found in multiple places.`
...
- Tone: substantial instructions require a role or persona. Ambient instructions
  are exempt; flag absence elsewhere. Use "Your job is…" or "You are…" to define
  the role.
...
- Consistency: flag steps, lists, or constructs that deviate from an established
  pattern.
...
- File references: relative to input file. Short-form links (e.g. `[README.md]`)
  are valid. Verify file existence when filesystem access is available.
...
- `disable-model-invocation: true`: set when the skill must only trigger via
  `/<name>`
