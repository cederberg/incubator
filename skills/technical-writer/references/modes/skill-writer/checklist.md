...
- [ ] `name` uses kebab-case and contains only lowercase letters, numbers, and
...
- [ ] `description` contains no hedging language ("typically", "usually",
      "generally", "may")
- [ ] If near-neighbour skills exist that overlap with this skill: `description`
      includes at least one explicit non-trigger condition
...
- [ ] Workflow steps are numbered or named; no prose narrative substitutes for
      ordered steps
- [ ] Every sub-agent step specifies the agent's role, the inputs it receives,
      and the artifact or output it produces
- [ ] No workflow step includes verbatim prompt text — specify what the prompt
      should cover, not the wording
...
- [ ] Every artifact produced by the skill has an exact file path, naming
      convention, or output template — either in an Output section or inline in
      the workflow step that produces it
...
- [ ] Each instruction earns its place: would the agent get this wrong without
      it? If not, remove it.
- [ ] Terminology is consistent throughout — the same concept is not referred to
      by different names
